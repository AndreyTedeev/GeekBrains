...
namespace WpfApp.ViewModel
{
    public class MainViewModel : ViewModelBase
    {
                ...
                if (_cmdLoadData == null)
                    _cmdLoadData = new LambdaCommand(
                        (p) =>
                        {
                            ...
                            Schedules = new ObservableCollection<Schedule>(_scheduleStorage.Items);
                        }
                    );
                return _cmdLoadData;
            }
                ...
                if (_cmdSendEmail == null)
                    _cmdSendEmail = new LambdaCommand(
                        (p) => _mailService
                            ...
                            .Send(SelectedSender, SelectedRecipient, SelectedEmail)
                        ,
                        (p) => SelectedServer != null
                            && SelectedSender != null
        ...
        public ICommand ScheduleEmailCommand
        {
            get
            {
                if (_cmdScheduleEmail == null)
                    _cmdScheduleEmail = new LambdaCommand(
                        (p) => Schedules.Add( new Schedule { 
        ...
        public MainViewModel(IMailService mailService, IServerStorage serverStorage,
            ISenderStorage senderStorage, IRecipientStorage recipientStorage, 
            IEmailStorage emailStorage, IScheduleStorage scheduleStorage)
        {
